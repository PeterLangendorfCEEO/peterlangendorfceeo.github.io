window.legoBluetooth = {
    ...
    _buildSetSpeed: function(bitMask, speed) { return new Uint8Array([this.MOTOR_SET_SPEED_COMMAND, bitMask, speed & 0xFF]); },
    _buildRunForDegrees: function(bitMask, degrees, direction) { return new Uint8Array([this.MOTOR_RUN_FOR_DEGREES_COMMAND, bitMask, degrees & 0xFF, (degrees >> 8) & 0xFF, (degrees >> 16) & 0xFF, (degrees >> 24) & 0xFF, direction ]); },
    _buildRun: function(bitMask, direction) { return new Uint8Array([this.MOTOR_RUN_COMMAND, bitMask, direction]); },
    _buildStop: function(bitMask) { return new Uint8Array([this.MOTOR_STOP_COMMAND, bitMask]); },
    ...
    runForDegreesSynced: async function(commands) {
        ...
        const waiters = commands.map(() => {
            return new Promise((resolve) => {
                const type = this.MOTOR_RUN_FOR_DEGREES_RESULT;
                if (!this._pending[type]) this._pending[type] = [];
                let t = setTimeout(() => resolve(null), 8000);
                this._pending[type].push((payload) => { clearTimeout(t); resolve(payload); });
            });
        });
        
        await this._safeWrite(combined);
