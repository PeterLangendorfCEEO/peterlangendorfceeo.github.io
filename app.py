    ...
    for move in move_set:
        ...
        try:
            if move in ["forward", "back"]:
                ...
                # Combine both motors into a single synchronized transmission bitmask
                active_bitmask = 0
                ...
                if active_bitmask > 0:
                    ...
                    if start_angle is None:
                        # Fallback to synced encoders if the IMU is asleep
                        turn_degrees = int(abs(target_offset) * TURN_MULTIPLIER)
                        commands = []
                        if not left_failed: commands.append({"bitMask": LEFT, "speedPercent": speed, "direction": turn_dir, "degrees": turn_degrees})
                        if not right_failed: commands.append({"bitMask": RIGHT, "speedPercent": speed, "direction": turn_dir, "degrees": turn_degrees})
                        
                        if len(commands) > 0:
                            js_commands = to_js(commands, dict_converter=window.Object.fromEntries)
                            await window.legoBluetooth.runForDegreesSynced(js_commands)
                    else:
                        # True Closed-Loop IMU turn via Javascript!
                        target_angle = normalize_angle(start_angle + target_offset)
                        await window.legoBluetooth.turnUntilAngle(active_bitmask, speed, turn_dir, target_angle)
